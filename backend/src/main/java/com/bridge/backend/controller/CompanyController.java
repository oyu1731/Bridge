...
public class CompanyController {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> withdrawCompany(@PathVariable Long id) {
        try {
    ...
    /**
     * 最新の企業を取得
     * GET /api/companies/recent?limit={limit}
     */
    @GetMapping("/recent")
    public ResponseEntity<List<CompanyDTO>> getRecentCompanies(@RequestParam(defaultValue = "10") int limit) {
        try {
            List<CompanyDTO> companies = companyService.getRecentCompanies(limit);
            return ResponseEntity.ok(companies);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    ...
    /**
     * 企業の存在確認
     * HEAD /api/companies/{id}
     */
    @RequestMapping(value = "/{id}", method = RequestMethod.HEAD)
    public ResponseEntity<Void> checkCompanyExists(@PathVariable Long id) {
        try {
            boolean exists = companyService.existsCompany(id);
