...
import 'dart:convert';
import 'package:screenshot/screenshot.dart'; // ★ 追加
...
class _ResultScreenState extends State<ResultScreen> {
  // ★ ScreenshotControllerを定義
  final ScreenshotController screenshotController = ScreenshotController();

  // _ResultScreenState クラス内

  Future<void> shareResultScreen() async {
    // 共有テキストの準備 (変更なし)
    ...
    final String shareBody =
        '私は${widget.courseType}コースで【${rankInfo.title}】ランクを獲得しました！正答率は${(correctRate * 100).toStringAsFixed(1)}%です。\nあなたも挑戦してみてください！';

    // ★ 件名/タイトルとして送りたいメッセージ（アプリによっては本文欄に採用される）
    final String shareSubject =
        '${widget.courseType}の結果：【${rankInfo.title}】ランク！';

    print('✅ 生成されたシェアメッセージ: $shareBody');

    try {
      final Uint8List? imageBytes = await screenshotController.capture();

      if (imageBytes != null) {
        // 1. 一時ディレクトリにファイルを保存 (より確実な共有のため)
        //    この部分には dart:io のパッケージが必要ですが、
        //    ここでは一時的なファイル名の生成と XFile の変換のみを行います。

        // XFileをバイトデータから直接生成
        final XFile imageFile = XFile.fromData(
          imageBytes,
          mimeType: 'image/png',
          name: 'quiz_result.png',
        );
        ...
        await Share.shareXFiles(
          [imageFile],
          text: shareBody, // ★ 本文
          subject: shareSubject, // ★ 件名 (本文が使われなかった場合に備える)
        );

        print('スクリーンショットとメッセージをシェアしました');
      } else {
        // 画像キャプチャに失敗した場合は、テキストのみ共有
        await Share.share(shareBody, subject: shareSubject);
        print('スクリーンショットのキャプチャに失敗したため、テキストのみシェアしました: $shareBody');
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // ★ body全体をScreenshotウィジェットでラップする
      body: Screenshot(
        controller: screenshotController, // コントローラーを関連付け
            ...
            children: [
              ...
              Column(
                children: [
                  ...
                  const SizedBox(height: 12),
                  OutlinedButton(
                    onPressed: shareResultScreen, // ★ ここで新しい関数を呼び出す
